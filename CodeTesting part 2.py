...
for j in all_words:
    for i in range(len(all_words[j])):  # 5
        if count < len(all_words[j]):  # 5
            if letter_in_word == all_words[j][count]:
                print("——————")
                print("| " + all_words[j][count] + " |🟢")
                print("——————")
                print()
        count += 1
...
while count < 5 and (again.upper() == "Y" or again == ""):
    print(f"Count: {count}")
    count += 1
else:
