...
print("\t\t \t\t\t\t\t\t\t\t\t\u001b[1m\u001b[40mYour Spotify Imagine Dragging These Nuts Playlist\033[0m")
print()
...
spaces = " "
for i in range(len(song_playlist_dict)):
    a, s = "\033[94m\u001b[1mArtist\033[0m", "\033[92m\u001b[1mSong\033[0m"
    for j in range(len(song_playlist_dict)):
        if j < 1:
            remainder = my_max - len(song_playlist_dict[i][j]) - len(song_playlist_dict[i][j+1])
            if remainder <= my_max:
                print(f"\t\t\t\t\t\t\t\t\t\t\033[1m{song_playlist_dict[i][j]}{spaces * (remainder-1)}{song_playlist_dict[i][j + 1]}\u001b[0m")
            else:
